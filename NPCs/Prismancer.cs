		...
		{if (npc.localAI[0] == 0f)
			...
			if(Main.rand.Next(450) == 5) {
				if(Main.player[npc.target].Center.Y <= npc.Center.Y) {
					Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0 - (TrajectoryX * 7), 0 - (TrajectoryY * 7), mod.ProjectileType("CultistFire"), 40, 0f, npc.target);
				} else if(Main.player[npc.target].Center.Y > npc.Center.Y) {
					Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (TrajectoryX * 7), TrajectoryY * 7, mod.ProjectileType("CultistFire"), 40, 0f, npc.target);
				}
			}
			...
			if(Main.rand.Next(575) == 5) {
				...
				do {  //Keep teleporting if too close to player
