...
namespace CrystiliumMod.Items.Weapons
{
    public class TrueSapphireStaff : ModItem
    {
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(mod.ItemType("EnchantedSapphireStaff"), 1);
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
			if(Main.myPlayer == player.whoAmI) {
				...
				Projectile.NewProjectile(mouse.X + Main.rand.Next(-80, 80), player.Center.Y - 350 + Main.rand.Next(-50, 50), 0, 20, mod.ProjectileType("SapphireDroplet"), damage, knockBack, player.whoAmI);
				Projectile.NewProjectile(mouse.X + Main.rand.Next(-80, 80), player.Center.Y - 350 + Main.rand.Next(-50, 50), 0, 20, mod.ProjectileType("SapphireDroplet"), damage, knockBack, player.whoAmI);
			}
			return false;
        }
