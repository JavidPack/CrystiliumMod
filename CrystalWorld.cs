...
namespace CrystiliumMod
{
	public class CrystalWorld : ModWorld
	{
		...
		public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
		{
			...
			tasks.Insert(ShiniesIndex +  1, new PassLegacy("CrystalBiomeGen", delegate(GenerationProgress progress)
			{
                ...
                for (int i = 0; i < (int)Main.maxTilesX / 900; i++)
				{
					...
					WorldGen.TileRunner(XvalueMid, YvalueMid, (double)WorldGen.genRand.Next(300,300), 1, mod.TileType("CrystalBlock"), false, 0f, 0f, true, true); //c = x, d = y
			/*		for (int A = Xvalue; A < XvalueHigh; A++)
					...
					for (int C = 0; C < 400; C++)
					{
						WorldGen.PlaceChest(Xvalue + WorldGen.genRand.Next(350, 450), Yvalue + WorldGen.genRand.Next(350, 450), (ushort)mod.TileType("CrystalChest"), false, 2);
					}
					for (int C = 0; C < 40; C++)
					{
		...
		public override void PostWorldGen()
		{
            ...
            {
                int[] itemsToPlaceInGlassChestsSecondary = new int[] { mod.ItemType("CrystalBottle"), mod.ItemType("ShinyGemstone"), mod.ItemType("RadiantPrism"), mod.ItemType("GeodeItem"), 73};
            ...
            int[] itemsToPlaceInGlassChests = new int[] { mod.ItemType("CrystalStaff"), mod.ItemType("CrystalEdge"), mod.ItemType("GemFury"), mod.ItemType("Geode"), mod.ItemType("PrismCast"), mod.ItemType("Glowstrike"), mod.ItemType("Sharpoon")};
